...
const EVENTS: Event[] = [
    ...
    },
    {
        ...
        photo       : "/fillingPic.gif",
        participants: 100,
        datedebut   : new Date("2026-02-04"),
        datefin     : new Date("2026-02-04"),
        lieu        : "B007",
...
export async function getListFutureEvents(): Promise<Event[]> {
    ...
    return EVENTS.map((event) => { return mapEvents(event); }).filter((event) => { return event.datefin > new Date(); });
}
